import type { WysiwyvEvaluatorFunction, WysiwyvPath } from "../type/engine.js";
  ...
  if (!isPlainObject($partial)) {
    ...
    for (const expectedKey in $partial) {
      if (!(expectedKey in value)) {
        errors.fault(
          errMissing(expectedKey, $partial[expectedKey], [
            ...path,
            expectedKey,
          ]),
        );
      ...
      const valueErrors = evaluate(expectedSub, candidateSub, [
        ...path,
        expectedKey,
      ]);
...
const validateEachElement = (
  ...
  for (const valueKey in value) {
    const eachErrors = evaluate($eachElement as HookValue, value[valueKey], [
      ...path,
      valueKey,
    ]);
