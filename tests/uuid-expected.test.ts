...
describe('UUID Expected JSON Object', () => {
  let wyv: WysiwyvInstance = makeWysiwyv();
  ...
  test('Invalid UUID format', () => {
    ...
    expect(result).toEqual([
      {
        message: "Expected value to be a valid UUID, got 'invalid-uuid'",
        path: ".id",
      }
    ]);
  });
  ...
  test('Invalid UUID version', () => {
    ...
    expect(result).toEqual([
      {
        message: "Expected value to be a valid UUID of version '4', got '6ba7b810-9dad-11d1-80b4-00c04fd430c8'",
        path: ".id",
      }
    ]);
  });
